...
function hideGloss(){ const t = $('glossTooltip'); if(t) t.classList.add('hidden'); }

// Click su parola = traduzione, SENZA attivare i bottoni risposta
document.addEventListener('click', (e)=>{
  const w = e.target && e.target.closest ? e.target.closest('.gloss-word') : null;
  if(w){
    e.preventDefault();
    e.stopPropagation();
    showGloss(w);
...
// Progressi (competenze) e streak
const SKILLS = ["HAY","MUCHO","PREPOSICIONES","DEICTICOS","FAMILIA","VERBI"];
let skillStats = Object.fromEntries(SKILLS.map(k => [k, {ok:0, total:0}]));
let streak = 0;
...
function resetProgress(){
  skillStats = Object.fromEntries(SKILLS.map(k => [k, {ok:0, total:0}]));
  streak = 0;
  const si = $("streakInfo");
  if(si) si.textContent = "Streak: 0";
  updateSkillsUI();
}

function updateSkillsUI(){
  for(const k of SKILLS){
    const s = skillStats[k] || {ok:0,total:0};
  ...
  const si = $("streakInfo");
  if(si) si.textContent = `Streak: ${streak}`;
}
...
function showToast(title, text){
  ...
  showToast._h = window.setTimeout(() => t.classList.add("hidden"), 3200);
}

function setFeedback({title, body, longText, kind}){
  const box = $("feedbackBox");
  if(!box) return;
  box.style.display = "block";
  $("feedbackTitle").textContent = title;
  $("feedbackText").innerHTML = body;
  const l = $("feedbackLong");
  if(l){
    l.textContent = longText || "";
    l.style.display = longText ? "block" : "none";
  }
...
function registerResult(tag, correct){
  const key = SKILLS.includes(tag) ? tag : null;
  if(key){
    skillStats[key].total += 1;
    if(correct) skillStats[key].ok += 1;
  }

  if(correct){
    streak += 1;
    if(streak === 15){
      showToast("🏅 Medaglia!", "15 risposte corrette di fila. Continua così!");
    }
  } else {
    streak = 0;
  }
...
function recomputeStatsFromAnswers(){
  ...
  for(let i=0;i<quiz.length;i++){
    ...
    if(SKILLS.includes(tag)){
      ...
      if(a.correct) skillStats[tag].ok += 1;
    }
  }
  updateSkillsUI();
}
...
function setExplain(show){ $('postit').style.display = show ? 'block' : 'none'; }

function renderQuestion(){
  const q = quiz[current];
  clearFeedback();
  $("meta").textContent = `Domanda ${current+1} / ${quiz.length} · Tipo: ${q.type.toUpperCase()} · Tema: ${q.tag} · Modalita: ${mode==='train'?'Allenamento':'Verifica'}`;

  // prompt
  $("prompt").innerHTML = wrapGloss(q.prompt);

  // post-it sempre visibile (aiuta)
  $("postitTitle").textContent = "Post-it grammaticale (in italiano)";
  $("postitText").innerHTML = renderPostit(q.postit);
  const extra = (q.type === 'open' && q.sample) ? ('<div class=\"muted\" style=\"margin-top:8px;\">Esempio: ' + escapeHtml(q.sample) + '</div>') : (q.type === 'fill' ? ('<div class=\"muted\" style=\"margin-top:8px;\">Suggerimento: ' + escapeHtml(q.hint||'') + '</div>') : ''); $('postitText').innerHTML = renderPostit(q.postit) + extra;
  setExplain(true);
  ...
  if(q.type === "mcq"){
    ...
    $("checkOpenBtn").addEventListener("click", ()=>{
      ...
      if(mode==="verify"){
        ...
        locked[current] = true;
        fb.textContent = res.ok ? "✅ Ok" : "❌";
        registerResult(q.tag, !!res.ok);

        if(res.ok){
          setFeedback({
            title: "✅ Bene!",
            kind: "good",
            body: `Hai usato correttamente la struttura richiesta.<br><span class="muted">${escapeHtml(res.msg||"")}</span>`,
            longText: ""
          });
        } else {
          setFeedback({
            title: "❌ Quasi!",
            kind: "bad",
            body: `Va bene la tua idea, ma controlla questo punto:<br><strong>${escapeHtml(res.msg||"Controlla la regola nel post-it.")}</strong>`,
            longText: q.sample ? `Esempio valido: ${q.sample}` : ""
          });
        }
        updateScore();
        renderIndex();
    ...
    $("checkFillBtn").addEventListener("click", ()=>{
      ...
      locked[current] = true;
      fb.textContent = ok ? "✅" : "❌";
      registerResult(q.tag, ok);
      if(ok){
        setFeedback({
          title: "✅ Ottimo!",
          kind: "good",
          body: `Hai coniugato correttamente il verbo: <strong>${escapeHtml(q.answers[0])}</strong>.`,
          longText: q.hint || ""
        });
      } else {
        setFeedback({
          title: "❌ Rivedi",
          kind: "bad",
          body: `Corretto: <strong>${escapeHtml(q.answers[0])}</strong>.`,
          longText: q.hint || ""
        });
      }
      updateScore();
...
function onAnswerMCQ(selectedIndex){
  ...
  if(correct){
    setFeedback({
      title: "✅ Ben fatto!",
      kind: "good",
      body: "Risposta corretta. Continua cosi!",
      longText: ""
    });
  } else {
    const right = q.choices[q.correctIndex] || "(corretto)";
    setFeedback({
      title: "❌ Quasi",
      kind: "bad",
      body: `La risposta corretta era: <strong>${escapeHtml(right)}</strong>.`,
      longText: "Guarda il post-it sopra per la regola."
    });
  }
...
function finish(){
  ...
  // aggiorna progressi anche in verifica (calcolo finale)
...
function buildSessionQuiz(){
  const count = parseInt($("countSel")?.value || "30", 10);
  const format = $("formatSel")?.value || "mixed";
  let pool = shuffle(BANK);
  if(format === "mcq") pool = pool.filter(q => q.type === "mcq");
  if(format === "open") pool = pool.filter(q => q.type === "open" || q.type === "fill");
  return pool.slice(0, Math.max(1, Math.min(count, TOTAL_BANK)));
}
...
function start(){
  running = true;
  $("results").style.display = "none";
  // ricostruisci se cambiate impostazioni prima di avviare
  if(!quiz.length){
    quiz = buildSessionQuiz();
    answers = Array.from({length: quiz.length}, () => ({}));
    locked  = Array.from({length: quiz.length}, () => false);
...
$("countSel")?.addEventListener("change", ()=>{ if(!running) restart(); });
$("formatSel")?.addEventListener("change", ()=>{ if(!running) restart(); });
