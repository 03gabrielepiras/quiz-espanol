...
/* -------------------- Device detect (PWA mobile-first) -------------------- */
...
function applyDeviceClass(){
  ...
  document.body.classList.remove('ios','android','other');
  document.body.classList.add(os);
  // piccola ottimizzazione UX: in standalone (PWA) evita selezioni/zoom strani
...
function pickBestSpanishVoice(){
  try{
    ...
    const prefer = ["monica","paulina","jorge","google","enhanced","spanish","espanol"];
    const score = (v)=>{
      ...
      if(v.default) s+=2;
      for(const p of prefer){ if(name.includes(p)) s+=6; }
      if(name.includes("robot")) s-=10;
      return s;
    };
...
if("speechSynthesis" in window){
  try{ window.speechSynthesis.onvoiceschanged = ()=>{ BEST_ES_VOICE = pickBestSpanishVoice(); }; }catch(e){}
}
...
function defaultProgress(){
  return {
    ...
    adjectives: {}, // {es:{seen,correct,it}}
    objects: {},    // {es:{seen,correct,it}}
...
function bumpAdj(progress, es, it, ok){
  if(!es) return;
  progress.adjectives ||= {};
  progress.adjectives[es] ||= { seen:0, correct:0, it: it||'' };
  progress.adjectives[es].seen += 1;
  if(ok) progress.adjectives[es].correct += 1;
}

function bumpObj(progress, es, it, ok){
  if(!es) return;
  progress.objects ||= {};
  progress.objects[es] ||= { seen:0, correct:0, it: it||'' };
  progress.objects[es].seen += 1;
  if(ok) progress.objects[es].correct += 1;
}
...
function buildC_hayEsta(){
  ...
  if(mode === 0){
    ...
    return cloze({
      ...
      meta:{ verb:'haber', obj: obj.es, objIt: obj.it }
    });
  }
  if(mode === 1){
    // está position
...
function recordAnswer(ok, q, userText, correctText){
  ...
  if(q.meta?.adj) bumpAdj(PROGRESS, q.meta.adj, q.meta.adjIt||'', ok);
  if(q.meta?.obj) bumpObj(PROGRESS, q.meta.obj, q.meta.objIt||'', ok);
...
function renderDashboard(){
  ...
  // aggettivi scoperti
  const adjRows = Object.entries(PROGRESS.adjectives || {})
    .map(([es,a])=>({es, it:a.it||'', seen:a.seen||0, correct:a.correct||0, pct:(a.seen?Math.round((a.correct/a.seen)*100):0)}))
    .sort((x,y)=> (x.pct - y.pct) || (y.seen - x.seen))
    .slice(0, 30)
    .map(a=>{
      const it = a.it ? `(${escapeHtml(a.it)})` : '';
      return `<div class="wordRow"><div class="wMain"><div class="wEs">${escapeHtml(a.es)}</div><div class="wIt">${it} • ${a.pct}% • ${a.correct}/${a.seen}</div></div><button class="sayBtn" type="button" data-say="${escapeHtml(a.es)}" aria-label="Pronuncia">🔊</button></div>`;
    }).join('');
  const objRows = Object.entries(PROGRESS.objects || {})
    .map(([es,o])=>({es, it:o.it||'', seen:o.seen||0, correct:o.correct||0, pct:(o.seen?Math.round((o.correct/o.seen)*100):0)}))
    .sort((x,y)=> (x.pct - y.pct) || (y.seen - x.seen))
    .slice(0, 30)
    .map(o=>{
      const it = o.it ? `(${escapeHtml(o.it)})` : '';
      return `<div class="wordRow"><div class="wMain"><div class="wEs">${escapeHtml(o.es)}</div><div class="wIt">${it} • ${o.pct}% • ${o.correct}/${o.seen}</div></div><button class="sayBtn" type="button" data-say="${escapeHtml(o.es)}" aria-label="Pronuncia">🔊</button></div>`;
    }).join('');

  const dashAdjs = document.getElementById('dashAdjs');
  if(dashAdjs) dashAdjs.innerHTML = adjRows || '<div class="muted">Gli aggettivi compaiono mentre fai esercizi.</div>';
  const dashObjs = document.getElementById('dashObjs');
  if(dashObjs) dashObjs.innerHTML = objRows || '<div class="muted">Gli oggetti/nomi compaiono mentre fai esercizi.</div>';
...
function bind(){
  // classi CSS per iPhone / Android (touch targets, safe-area, ecc.)
  try{ applyDeviceClass(); }catch{}
  setupNav();
  ...
  // pronuncia singola parola (progressi)
  document.addEventListener('click', (e)=>{
    const btn = e.target && e.target.closest ? e.target.closest('.sayBtn') : null;
    if(!btn) return;
    e.preventDefault();
    e.stopPropagation();
